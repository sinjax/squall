...
public class BaseAtomFilterFunction extends BaseFilterFunction {
	 ...
	 * @param <T>
	 */
	public static class RuleWrappedAtomFilter extends RuleWrappedFunction<BaseAtomFilterFunction> {
		
		protected RuleWrappedAtomFilter(Functor clause){
			super(new AtomFiltARVH(clause));
			this.wrap(new BaseAtomFilterFunction(((AtomFiltARVH) super.getVariableHolder()).clause));
		}
